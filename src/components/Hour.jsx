...
import { BsFillCloudRainHeavyFill, BsCloudRainFill, BsSunFill, BsEmojiSunglasses } from "react-icons/bs";
...
const Hour = ({hourData}) => {
    const getWeatherIcon = (temp) => {
        if (temp < 10) {
            ...
            return <BsCloudRainFill size={32} color='white'/>
        } else if (temp < 20) {
            return <BsSunFill size={32} color='white'/>
        } else {
  ...
  return (
    <Paper elevation={3} sx={{padding: 2, margin: 1, color: 'white', backgroundColor: '#333333'}} >
        <Box
            display="flex"
            ...
            mt={1}>
                <Box mr={1}>
                    {getWeatherIcon(hourData.temp)}
                </Box>
                <Box mx={1}>
                    {getTrendIcon(hourData.arrow)}
                </Box>
